        ...
        <div class="container">
            ...
            <ul class="list">
                ...
                <li class="card" v-for="item in shopList" :key="item.id">
                    <div class="pic">
                        <img referrerpolicy="never" :src="item.pic" alt="">
                    </div>
                    <div class="desc">
...
import { getHomeShop } from '@/api/index.js'
let shopList = reactive([]);
let shop_page = (0);

// 更换
const changeShopList=()=>{
    shop_page ++;
    // console.log('商品列表---page', shop_page);
    GetgetHomeShop();
}

// 获取商品数据
const GetgetHomeShop = async (shop_page) => {
    let res = await getHomeShop( {shop_page: shop_page} )

    shopList = res.data.data;
    // console.log('商品列表---', shopList);
}
